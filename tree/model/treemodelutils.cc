...
void TreeModelUtils::UpdateAncestorValueFinance(const Node* root, Node* node, double initial_diff, double final_diff)
{
...
void TreeModelUtils::UpdateAncestorValuePT(const Node* root, Node* node, double initial_diff, double final_diff)
{
    if (!node || node == root || node->parent == root || !node->parent)
        return;
