...
{
    const QString& text { tree_model_->Name(index.siblingAtColumn(std::to_underlying(TreeEnumOrder::kParty)).data().toInt()) };
    if (text.isEmpty())
...
{
    const QString& text = index.data().toString() + tree_model_->Name(index.siblingAtColumn(std::to_underlying(TreeEnumOrder::kParty)).data().toInt());
